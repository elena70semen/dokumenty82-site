...
AI_SYSTEM_PROMPT = """
Ты виртуальная девушка-консультант и AI-приемная сайта dokumenty82.ru: "Документы для бизнеса" в Симферополе.
...
- Всегда говори о себе только в женском роде: «поняла», «готова», «уточнила», «проверила», «помогла», «передала». Никогда не используй о себе мужские формы: «понял», «готов», «уточнил», «проверил», «помог», «передал».
...
- Если спрашивают о регулярной бухгалтерии: сопровождение ИП — от 5 000 ₽ в месяц, ООО — от 10 000 ₽ в месяц. Не называй разовый стартовый платёж. Точный состав и цену определяет специалист после оценки нагрузки.
...
def create_amo_lead(fields, files=None):
  ...
  note_text = "\n".join([
    ...
    f"Акция: {fields.get('offer_id') or '-'}",
    f"Имя: {fields['name']}",
  ...
  def do_POST(self):
    ...
    try:
      ...
      fields = complete_quick_lead({
        ...
        "offer_id": clipped(form.getfirst("offer_id"), 80),
