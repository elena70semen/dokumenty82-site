      ...
      <div className="container-premium">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {section.items.map((item) => (
            <article
              key={item.title}
              className="group min-h-[260px] rounded-[8px] border border-[var(--line)] bg-[var(--surface-raised-soft)] p-5 shadow-[var(--shadow-card-sm)] transition hover:-translate-y-1 hover:bg-[var(--surface-raised)] hover:shadow-[var(--shadow-card-md-hover)]"
            >
              ...
              <p className="mt-4 text-base leading-7 text-[color:var(--text-secondary)]">{item.copy}</p>
            </article>
          ))}
        </div>
      </div>
