...
export const routePageData: Record<(typeof routePageSlugs)[number], RoutePageConfig> = {
  "razbor-situacii": {
    ...
    hero: {
      ...
      title: "Разберём ситуацию и подскажем документальный маршрут",
      text:
        "Если непонятно, какая страница подходит, начните с разбора: опишите ситуацию, назовите документ или запрос, а мы подскажем безопасный следующий шаг.",
      icon: "question",
      badgeKind: "route",
    ...
    situation: {
      ...
      text: "Разбор нужен для смешанных, чувствительных или просто непонятных вопросов, где важно не выбрать неверную страницу и не передать лишние данные.",
      items: [
        {
          title: "Пришло письмо или запрос",
          copy: "Можно назвать источник, тему и формулировку без передачи лишних деталей.",
          icon: "document",
          badgeKind: "document"
        },
        {
          title: "Тем несколько сразу",
          copy: "ИФНС, банк, отчётность, адрес, регистрация или кадры могут пересекаться в одном вопросе.",
          icon: "route",
          badgeKind: "route"
        },
        {
    ...
    scope: {
      ...
      text: "Первый шаг фиксирует вводные, границы вопроса и следующий документальный ход. Подготовка документов начинается после уточнения ситуации.",
    ...
    process: {
      ...
      steps: [
        ...
        { title: "Фиксируем следующий шаг", copy: "Понимаете, что показать, какую страницу открыть и что уточнить безопасно." }
      ]
    },
    documents: {
      id: "show-documents",
  ...
  kontakty: {
    ...
    situation: {
      ...
      title: "Здесь только связь, адрес и безопасный следующий шаг",
      text: "Страница не превращается в каталог услуг. Она помогает связаться, приехать или согласовать способ показа документов.",
      items: [
        {
        ...
        },
        {
          ...
          copy: "Адрес указан текстом, чтобы маршрут был понятен даже без внешней карты.",
          icon: "location",
          badgeKind: "office"
        },
        {
  ...
  otchetnost: {
    ...
    hero: {
      ...
      title: "Отчётность: выбрать точный документальный маршрут",
      text:
    ...
    situation: {
      ...
      text: "Хаб не забирает точные интенты дочерних страниц. Он помогает выбрать страницу по форме бизнеса, периоду и источнику вопроса.",
      items: [
        {
          title: "УСН или нулевая отчётность",
          copy: "Разводим декларацию, нулевую отчётность ООО и нулевую отчётность ИП без смешения задач.",
          icon: "reporting",
          badgeKind: "document"
        },
        {
          title: "Не хватает данных",
          copy: "Если документы или учётные сведения неполные, может подойти восстановление учёта, а не точная отчётная страница.",
          icon: "folder",
          badgeKind: "office"
        },
        {
  ...
  "bank-i-115-fz": {
    ...
    href: "/bank-i-115-fz/",
    kind: "hub",
    title: "Банковский запрос или пакет по 115-ФЗ",
    ...
    situation: {
      ...
      items: [
        ...
        },
        {
          ...
          copy: "Если банковский вопрос затрагивает отчётность, ИФНС или адрес, лучше начать с общего разбора.",
          icon: "route",
          badgeKind: "route"
        }
      ]
    ...
    documents: {
      ...
      text: "Не нужно собирать всё сразу. Сначала важно понять, это один запрос банка или более широкий пакет по ситуации.",
      items: [
