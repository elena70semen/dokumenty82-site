...
const routePath = "/adres-egryul-direktor/";
const mimeTypes = {
...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(await Promise.all(["h1", "h2", "h3"].map((tag) => page.locator(`main ${tag}`).count())), [1, 7, 16]);
    assert.equal((await page.locator("main h1").innerText()).replace(/\s+/g, " ").trim(), "Адрес, ЕГРЮЛ и изменения в компании");
    assert.equal(await page.locator("main form").count(), 0);
    assert.equal(await page.locator("main .button-primary").first().getAttribute("href"), "/razbor-situacii/");
    assert.equal(await page.locator("main .button-secondary").first().getAttribute("href"), "/kontakty/");
    assert.equal(await page.locator('meta[name="robots"]').getAttribute("content"), "index, follow");
    assert.equal(await page.locator('link[rel="canonical"]').getAttribute("href"), "https://dokumenty82.ru/adres-egryul-direktor/");
