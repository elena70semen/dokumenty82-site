...
const routePath = "/sdacha-otchetnosti-ip/";
const mimeTypes = {
  ...
  try {
    ...
    await context.route("**/*", async (route) => {
      ...
      if (url.origin === origin && url.pathname === "/api/lead") {
        ...
        assert.match(body, /name="source_page"[\s\S]*\/sdacha-otchetnosti-ip\//);
        assert.match(body, /name="task_type"[\s\S]*Сдача отчётности ИП/);
    ...
    assert.deepEqual(await Promise.all(["h1", "h2", "h3"].map((tag) => page.locator(`main ${tag}`).count())), [1, 10, 27]);
    assert.equal((await page.locator("main h1").innerText()).replace(/\s+/g, " ").trim(), "Сдача отчётности ИП в Симферополе");
    ...
    assert.equal(await page.locator('link[rel="canonical"]').getAttribute("href"), "https://dokumenty82.ru/sdacha-otchetnosti-ip/");
    ...
    await form.locator('textarea[name="message"]').fill("Отчётность ИП за период");
    await form.locator('input[name="privacy"]').check();
    await form.locator('button[type="submit"]').click();
    await form.locator('[role="status"]').filter({ hasText: /отправлена/i }).waitFor({ timeout: 5000 });

    assert.equal(submissions.length, 1);
    assert.deepEqual([...new Set(broken)], []);
    assert.deepEqual(consoleErrors, []);
    console.log(JSON.stringify({ result: "PASS", route: routePath, metrics, formSubmission: true }, null, 2));
  } finally {
    await browser.close();
  }
