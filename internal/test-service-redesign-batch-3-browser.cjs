...
const specs = [
  { route: "/dokumenty-dlya-banka-115-fz/", h1: "Документы для банка по 115-ФЗ в Симферополе", headings: [1, 8, 21], form: true },
  { route: "/otvet-na-zapros-banka/", h1: "Ответ на запрос банка по 115-ФЗ в Симферополе", headings: [1, 8, 21], form: true },
  { route: "/bank-i-115-fz/", h1: "Помощь по 115-ФЗ при запросе банка в Симферополе", headings: [1, 8, 23], form: false },
  { route: "/srochnye-voprosy/", h1: "Срочная помощь с документами бизнеса", headings: [1, 10, 27], form: true },
  { route: "/registraciya-i-likvidaciya/", h1: "Регистрация, изменения и прекращение бизнеса в Симферополе", headings: [1, 8, 21], form: false },
];
const mimeTypes = {
  ...
  try {
    ...
    await context.route("**/*", async (route) => {
      ...
      if (url.origin === origin && url.pathname === "/api/lead") {
        ...
        assert.match(body, /name="source_page"[\s\S]*\/(?:dokumenty-dlya-banka-115-fz|otvet-na-zapros-banka|srochnye-voprosy)\//);
    ...
    for (const spec of specs) {
      ...
      assert.equal(await page.locator("main form").count(), spec.form ? 1 : 0);
      ...
      results.push({ route: spec.route, widths: [1440, 1024, 768, 390, 320], form: spec.form });
    }
    assert.equal(submissions.length, specs.filter((spec) => spec.form).length);
