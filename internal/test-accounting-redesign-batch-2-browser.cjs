...
const specs = [
  { route: "/buhgalterskoe-soprovozhdenie-ooo/", h1: "Бухгалтер для ООО в Симферополе", headings: [1, 11, 22], taskType: "Бухгалтерское сопровождение ООО" },
  { route: "/vosstanovlenie-buhucheta/", h1: "Восстановление бухучёта", headings: [1, 9, 23], taskType: "Восстановление бухгалтерского учёта" },
];
const mimeTypes = {
  ...
  try {
    ...
    await context.route("**/*", async (route) => {
      ...
      if (url.origin === origin && url.pathname === "/api/lead") {
        ...
        const pageSpec = specs.find((spec) => body.includes(spec.route));
        assert.ok(pageSpec, "submitted source_page is unknown");
        assert.ok(body.includes(pageSpec.taskType), "task_type was not retained");
        assert.match(body, /name="phone"/);
        assert.match(body, /name="privacy"[\s\S]*\r\n1\r\n/);
        assert.doesNotMatch(body, /filename=/);
        submissions.push(pageSpec.route);
    ...
    for (const spec of specs) {
      ...
      await form.locator('textarea[name="message"]').fill("Тестовая консультация");
      ...
      results.push({ route: spec.route, metrics });
    }
    assert.deepEqual(submissions.sort(), specs.map((spec) => spec.route).sort());
